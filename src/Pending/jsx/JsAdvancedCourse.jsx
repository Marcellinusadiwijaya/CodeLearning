...
import { jsAdvancedCourse } from "./JsAdvancedCourse";
import { useState, useEffect } from "react";
import "../CSS/JsAdvanced.css";

export default function JsAdvancedCourse() {
  ...
  const section = jsAdvancedCourse[secIndex];
  const item = section.items[itmIndex];

  const currentUser = localStorage.getItem("currentUser");
  const courseId = "js-advanced";
  ...
  const isUnlocked = () => {
    ...
    const prevSection = jsAdvancedCourse[secIndex - 1];
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/js-advanced-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/js-advanced");
    }
  };
