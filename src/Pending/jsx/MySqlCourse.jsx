...
import { mySqlCourse } from "./MySqlCourse";
import { useState, useEffect } from "react";
import "../CSS/MySqlCourse.css";
...
export default function MySqlCourse() {
  ...
  const section = mySqlCourse[secIndex];
  const item = section?.items?.[itmIndex];

  const currentUser = localStorage.getItem("currentUser");
  const storageKey = `progress_mysql_${currentUser}`;
  ...
  const isUnlocked = () => {
    ...
    const prevSection = mySqlCourse[secIndex - 1];
  ...
  // redirect kalau ke-lock
  useEffect(() => {
    if (!isUnlocked()) {
      navigate("/course/mysql");
    }
  }, [sectionIndex, itemIndex, progress]);

  // reset state tiap pindah
  useEffect(() => {
    setSelected(null);
    setIsCorrect(null);
    setUserCode("");
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/mysql-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/mysql");
    }
  };
  ...
  if (!section || !item) return <p>Loading...</p>;
