...
export default function NodejsCourse() {
  ...
  const section = nodeJsCourse[secIndex];
  const item = section?.items?.[itmIndex];

  const currentUser = localStorage.getItem("currentUser");
  const storageKey = `progress_nodejs_${currentUser}`;
  ...
  const isUnlocked = () => {
    ...
    const prevSection = nodeJsCourse[secIndex - 1];
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/nodejs-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/nodejs");
    }
  };
