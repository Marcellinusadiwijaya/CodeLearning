...
export default function PhpCourse() {
  ...
  const section = phpCourse[secIndex];
  const item = section?.items?.[itmIndex];

  const currentUser = localStorage.getItem("currentUser");
  const storageKey = `progress_php_${currentUser}`;
  ...
  const isUnlocked = () => {
    ...
    const prevSection = phpCourse[secIndex - 1];
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/php-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/php");
    }
  };
