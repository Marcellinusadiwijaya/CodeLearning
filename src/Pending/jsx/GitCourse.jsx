...
export default function GitCourse() {
  ...
  const section = gitCourse[secIndex];
  ...
  const isUnlocked = () => {
    ...
    const prevSection = gitCourse[secIndex - 1];
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/git-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/git");
    }
  };
