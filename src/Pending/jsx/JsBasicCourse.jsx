...
import { jsBasicCourse } from "./JsBasicCourse";
import { useState, useEffect } from "react";
import "../CSS/JsBasic.css";

export default function JsBasicCourse() {
  ...
  const section = jsBasicCourse[secIndex];
  const item = section.items[itmIndex];

  const currentUser = localStorage.getItem("currentUser");
  const courseId = "js-basic";
  ...
  const isUnlocked = () => {
    ...
    const prevSection = jsBasicCourse[secIndex - 1];
  ...
  const goNext = () => {
    ...
    if (nextItem < section.items.length) {
      navigate(`/js-basic-course/${secIndex}/${nextItem}`);
    } else {
      navigate("/course/js-basic");
    }
  };
